...
from sklearn.metrics import accuracy_score, confusion_matrix, classification_report, mean_absolute_error, mean_squared_error, precision_score, cohen_kappa_score, confusion_matrix, plot_confusion_matrix, plot_roc_curve, plot_precision_recall_curve
...
"""Read the dataframe. Testing different dataframes"""
...
features = df_Angular_alt
features_names = list(features.drop(columns=["target"], axis=1).columns)
...
def eval_val(model, title):
    ...
    cks = cohen_kappa_score(y_val, pred)
    ...
    eval_val = {"Model":[title], "Score":[accuracy], "CKS":[cks], "MSE":[mse], "MAE":[mae], "Var":[variance], "Bias":[bias]}
...
#df_merge = pd.concat([ADC_DF, DTC_DF, XGB_DF, MLP_DF, RF_DF])
...
def eval_test(model, title):
    ...
    #plt.figure()
    skplt.estimators.plot_feature_importances(model, feature_names=features_names, title="XGB", x_tick_rotation=90)#X_train.columns, title="XGB", x_tick_rotation=90)
    plt.tight_layout()
